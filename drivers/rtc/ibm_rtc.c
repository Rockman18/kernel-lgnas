...
static int ibm_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	DBG2(dev, "ibm_rtc_read_alarm" NL);
...
{
	unsigned char reg;
	struct ibm_rtc_instance *rtc = dev_get_drvdata(dev);
	struct rtc_time *tm = &alrm->time;

	DBG2(dev, "ibm_rtc_set_alarm" NL);
...
{
	unsigned char reg;
	struct ibm_rtc_instance *dev = dev_get_drvdata((struct device *)dev_id);
...
static int ibm_rtc_probe(struct platform_device *ofdev,
			const struct of_device_id *match)
{
	unsigned char reg;
	struct ibm_rtc_instance *dev = NULL;
	...
	/* Allocate RTC device */
	dev = kzalloc(sizeof(struct ibm_rtc_instance), GFP_KERNEL);
	if (dev == NULL) {
		dev_err(&ofdev->dev, "kzalloc failed for dev\n");
		err = -ENOMEM;
		goto out;
	}
...
static int ibm_rtc_remove(struct platform_device *ofdev)
{
	struct ibm_rtc_instance *dev = platform_get_drvdata(ofdev);
