...
static phys_addr_t splice_src_dma_addrs[PIPE_DEF_BUFFERS];
static dma_addr_t splice_dst_dma_addrs[PIPE_DEF_BUFFERS];
...
int splice_dma_memcpy(struct splice_dma_desc *sd_p, unsigned int len)
{
	...
	res = ppc460ex_enable_burst(adev, dma_ch_id);
	if(unlikely(res != DMA_STATUS_GOOD)) {
		res = -EFAULT;
		printk("%s:%s:%d\n"
			"res=%d\n", __FUNCTION__, __FILE__, __LINE__, res);
	...
	for(i=0; i<sd_p->n_elems; i++) {
		...
		mapped++;
		res = ppc460ex_add_dma_sgl(adev, handle_p, dma_src, dma_dest, size);
		if(unlikely(res)) {
			printk("%s:%s:%d\n"
					"res=%d\n", __FUNCTION__, __FILE__, __LINE__, res);
			goto cleanup;
	...
	res = ppc460ex_enable_dma_sgl(adev, handle_p);
	if(unlikely(res)) {
		printk("%s:%s:%d\n"
			"Enable dma channel failed res=%d\n", __FUNCTION__, __FILE__, __LINE__, res);
		res = -EFAULT;
		goto cleanup;
	}
	...
	if(unlikely(res)) {
		...
		/* Check the error status bits */
		if(unlikely(mfdcr(DCR_DMA2P40_SR) & (1 << (23 - dma_ch_id)))) {
			printk(KERN_ERR "Error occured in the channel %d\n",dma_ch_id);
			printk("DCR_DMA2P40_SR=0x%x\n",mfdcr(DCR_DMA2P40_SR));
		}
	}

cleanup:
	mtdcr(DCR_DMA2P40_SR, DMA_STATUS_MASK_BYCHAN(dma_ch_id));
	//mtdcr(DCR_DMA2P40_SR, 0xFFFFFFFF);
