		...
		unsigned long flags)
{
	...
	hw_desc->cnt.etie = 0; /* EOT interrupt enable */
...
/*
 * apm82181_memcpy_complete_wait: make sure the tx will finish and caus an INT 
 */
static void apm82181_memcpy_complete_wait(apm82181_ch_t *chan,
			apm82181_desc_t *sw_desc)
{
        unsigned long int_wait_timeout = jiffies + msecs_to_jiffies(500);

        if(!test_bit(APM82181_DESC_INT, &sw_desc->flags)){
                apm82181_dma_put_desc(chan, sw_desc);
                return;
        }
        wait = 1;
        apm82181_dma_put_desc(chan, sw_desc);
        do {
                if (time_after_eq(jiffies, int_wait_timeout)) {
			/* not sure if the the tx is completed or not
			   Redo the DMA */
			apm82181_dma_put_desc(chan, sw_desc);	
                        return;
                }
...
static inline void apm82181_chan_append(apm82181_ch_t *chan)
{
	...
	case APM82181_PDMA3_ID:
		...
		list_for_each_entry_continue(iter, &chan->chain, chain_node) {
			if(test_bit(APM82181_DESC_FENCE, &iter->flags)){
				local_irq_restore(flags);
				/* RAID biodrain ops need to be finished first */
                        	apm82181_memcpy_complete_wait(chan, iter);
                        	local_irq_save(flags);
			} else
				apm82181_dma_put_desc(chan, iter);
...
static void __apm82181_adma_slot_cleanup(apm82181_ch_t *chan)
{
	...
	list_for_each_entry_safe(iter, _iter, &chan->chain,
                                        chain_node) {
		DBG("slots in chain%d: %d\n", chan->device->id, iter->idx);
	}
#endif
	list_for_each_entry_safe(iter, _iter, &chan->chain,
					chain_node) {
