/*
 * arch/arm/mach-kirkwood/nt11-setup.c
 *
 * LG AMS Setup
 *
	...
	int pmp, unsigned char cmnd, int force, int power)
{
	...
	if(power)
	{
		if(!level)
		{
			...
			/* sata interface control register */
	...
	else
	{
		if(level)
		{
			...
			save_reg[port_no] = readl(port_mmio + 0xc);
			writel(0x0, port_mmio + 0xc);
			__disable_sata_pm(port_no);
...
static struct mv_udc_platform_data lgnas_udc_data =
{
...
static u64 udc_dmamask = 0xffffffff;

static struct resource lgnas_udc_resources[] =
{
	[0] = {
		.start = USB_PHYS_BASE,
...
static struct platform_device lgnas_device_udc =
{
	.name	= "mv_udc",
	.id = 0,
	.dev = {
		...
		.platform_data = &lgnas_udc_data,
	},
	.resource	= lgnas_udc_resources,
	.num_resources = ARRAY_SIZE(lgnas_udc_resources),
};

static void __init lgnas_udc_init(void)
{
	platform_device_register(&lgnas_device_udc);
}
...
#define LGAMS_MACHINE_NAME	"LG Advanced Multimedia Storage NT1"
