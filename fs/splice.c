			   ...
			   splice_actor *actor)
{
	...
	for (;;) {
		if (pipe->nrbufs) {
			...
			kunmap(page);
			ret = pagecache_write_end(file, mapping, page_pos, splice_size, splice_size,
					page, fsdata);
			if( (unlikely(ret < 0)) || (unlikely(ret < splice_size))) {
				printk("%s:%s:%d\n"
						"ret = %d\n", __FUNCTION__, __FILE__, __LINE__, ret);
				if(ret > 0)
					retval = -EAGAIN;
			   ...
			   splice_actor *actor)
{
	...
	pipe_addr_map = kzalloc(sizeof(*pipe_addr_map) * pipe->buffers, GFP_ATOMIC);
	if(unlikely(!pipe_addr_map)) {
		retval = -ENOMEM;
		printk(KERN_ERR "%s:%s:%d\n"
			"returning ENOMEM\n", __FUNCTION__, __FILE__, __LINE__);
		goto cleanup;
...
//	int nrbufs = pipe->nrbufs;
	...
	//pipe->curbuf = (pipe->curbuf + pipe->nrbufs) & (pipe->buffers - 1);
	//pipe->nrbufs = 0;
	ret = len;
...
out:
	...
	/* 
	 * pipe->bufs was dynamically allocated in alloc_pipe_info
	 * so we need to free it before freeing pipe.
	 */
	kfree(pipe->bufs);
   	kfree(pipe);
	return ret;
